...
sf::Vector2f BezierExercise::B(std::vector<sf::Vector2f> coorArr, int i, int j, float at) {
    if (j == 0) {
        return coorArr[i];
    }
    else {
...
std::vector<sf::Vector2f> BezierExercise::getPolylineBesier(std::vector<sf::Vector2f> controllPoints, int stepCount) {
...
void BezierExercise::UpdateModelView() {
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
}

void BezierExercise::UpdateProjection() {
